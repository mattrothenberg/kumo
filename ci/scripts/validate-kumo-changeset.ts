...
import { hasChangesInPath, getNewlyAddedFiles, isMergeRequestContext, logMergeRequestContext } from '../utils/git-operations';
...
function main() {
  ...
  if (!hasKumoChanges) {
    console.log('No changes detected in packages/kumo/, skipping changeset validation');
    return;
  }
  ...
  console.log(`✅ Found ${newKumoChangesets.length} NEW changeset(s) for @cloudflare/kumo:`);
...
function checkForKumoChanges(): boolean {
  ...
  if (result === null) {
    console.warn('⚠️  Warning: Could not determine if kumo changes exist, assuming they do');
    return true;
  }
...
function getNewlyAddedChangesets(): ChangesetFile[] {
  ...
  if (newFiles.length === 0) {
    console.warn('Warning: Could not determine newly added changesets, falling back to all changesets');
  ...
  for (const { status, path: filePath } of newFiles) {
    ...
    if (!fileName || fileName === 'config.json' || fileName === 'README.md' || fileName === 'USAGE.md' || !fileName.endsWith('.md')) {
      continue;
    }
    ...
    try {
      ...
      console.warn(`Warning: Could not parse changeset file ${fileName}: ${error}`);
    }
  }
...
function getChangesets(): ChangesetFile[] {
  ...
  for (const file of files) {
    // Skip config files and README
    if (file === 'config.json' || file === 'README.md' || file === 'USAGE.md' || !file.endsWith('.md')) {
      continue;
    }
