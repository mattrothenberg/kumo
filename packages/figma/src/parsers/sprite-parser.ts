...
export function parseSpriteIcons(): IconData[] {
  // Resolve sprite path - reference sibling kumo package
  ...
  try {
    ...
    // Go up: parsers -> src -> figma -> packages -> kumo
    spritePath = join(
      __dirname,
      "..",
      "..",
    ...
    // Fallback: assume running from packages/figma directory
    spritePath = join(process.cwd(), "../kumo/src/assets/icons/sprite.svg");
  }
