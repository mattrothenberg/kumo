...
describe("LayerCard Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   * SNAPSHOT TESTS - Regression guards for intermediate data
   *
   * These tests capture the intermediate data (dimensions, color bindings,
   * sub-component config) BEFORE it hits Figma APIs. This enables:
   *
   * 1. Testing without Figma plugin runtime
   * 2. Detecting unintended changes in layout logic or color bindings
   * 3. Validating the full source of truth chain:
   *    layer-card.tsx → component-registry.json → layer-card.ts → Figma
   *
   * If these snapshots change unexpectedly, it means:
   * - LayerCard component structure changed in layer-card.tsx (intended)
  ...
  it("should produce consistent sub-component configuration", () => {
    const subComponentConfig = getLayerCardSubComponentConfig();
    expect(subComponentConfig).toMatchSnapshot();
  });

  it("should produce consistent content configuration", () => {
    const contentConfig = getLayerCardContentConfig();
    expect(contentConfig).toMatchSnapshot();
  });
   ...
   * layer-card.ts computes before making any Figma API calls. It's the
