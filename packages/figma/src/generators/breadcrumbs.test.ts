...
describe("Breadcrumbs Generator - Color Bindings", () => {
  ...
  it("should use semantic color tokens", () => {
    const bindings = getBreadcrumbsColorBindings();
    expect(bindings.link).toBe("text-color-muted");
    expect(bindings.current).toBe("text-color-surface");
    expect(bindings.separator).toBe("text-color-disabled");
  });
});
...
describe("Breadcrumbs Generator - Separator Configuration", () => {
  ...
  it("should use ph-caret-right icon", () => {
    const config = getBreadcrumbsSeparatorConfig();
    expect(config.iconName).toBe("ph-caret-right");
  });
...
describe("Breadcrumbs Generator - Snapshots", () => {
  it("should match size config snapshot", () => {
    const config = getBreadcrumbsSizeConfig();
  ...
  it("should match base size snapshot", () => {
    const parsed = getBreadcrumbsParsedSizeStyles("base");
    expect(parsed).toMatchSnapshot();
  });

  it("should match color bindings snapshot", () => {
    const bindings = getBreadcrumbsColorBindings();
    expect(bindings).toMatchSnapshot();
  });

  it("should match separator config snapshot", () => {
    const config = getBreadcrumbsSeparatorConfig();
    expect(config).toMatchSnapshot();
  });

  it("should match complete data snapshot", () => {
    const allData = getAllBreadcrumbsData();
