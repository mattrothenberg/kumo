 ...
 * - Radio indicator: 16x16 circle, bg-surface (off) / bg-surface-inverse (on)
 * - Inner dot: 8x8 circle, bg-surface (visible when checked)
 *
...
export function getRadioBgVariable(checked: boolean): string {
  return checked ? "color-surface-inverse" : "color-surface";
}
...
function createRadioIndicator(
  ...
  if (checked) {
    ...
    // Inner dot is bg-surface (white in dark mode, contrasts with bg-surface-inverse)
    const dotBgVar = getVariableByName("color-surface");
    if (dotBgVar) {
