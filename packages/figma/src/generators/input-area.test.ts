...
describe("InputArea Generator - Ring Variable Logic", () => {
  ...
  it("should return color-active for default variant + focus state", () => {
    const ringVar = getInputAreaRingVariable("default", "focus");
    expect(ringVar).toBe("color-active");
  });

  it("should return color-error for error variant + focus state", () => {
    const ringVar = getInputAreaRingVariable("error", "focus");
    expect(ringVar).toBe("color-error");
  });

  it("should return correct ring variable for error variant + default state", () => {
    const ringVar = getInputAreaRingVariable("error", "default");
    expect(typeof ringVar).toBe("string");
    expect(ringVar.length).toBeGreaterThan(0);
  });
...
describe("InputArea Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   * SNAPSHOT TESTS - Regression guards for intermediate data
   *
   * These tests capture the intermediate data (size configs, variant configs,
   * state styles) BEFORE it hits Figma APIs. This enables:
   *
   * 1. Testing without Figma plugin runtime
   * 2. Detecting unintended changes in configuration or logic
   * 3. Validating the complete data structure
   *
   * If these snapshots change unexpectedly, it means:
   * - InputArea generator configuration changed (review carefully)
  ...
  it("should produce consistent state configuration", () => {
    const stateConfig = getInputAreaStateConfig();
    expect(stateConfig).toMatchSnapshot();
  });

  it("should produce consistent withLabel configuration", () => {
    const withLabelConfig = getInputAreaWithLabelConfig();
    expect(withLabelConfig).toMatchSnapshot();
  });

  it("should produce consistent dimensions for xs size", () => {
    const dims = getInputAreaSizeDimensions("xs");
   ...
   * input-area.ts computes before making any Figma API calls. It's the
   * most comprehensive regression guard.
   */
  it("should produce consistent complete data for all variants (golden path)", () => {
    const allData = getAllInputAreaVariantData();
