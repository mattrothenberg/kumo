...
const inputDir = path.join(cwd, 'dist', 'icons', 'svgo');
const inputDirRelative = path.relative(cwd, inputDir);
const outputDir = path.join(cwd, 'dist', 'icons');
(async () => {
...
async function generateIconFiles() {
  ...
  const typeOutputFilepath = path.join(outputDir, 'index.ts');
...
Everything in this directory is generated by \`npm run icons:all\`.
`,
  );
...
async function writeIfChanged(filepath: string, newContent: string) {
  ...
  await $.command(`prettier --write ${filepath} --ignore-unknown`);
  return true;
}
