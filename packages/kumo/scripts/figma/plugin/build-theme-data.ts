 ...
 *
 * Usage:
 *   npx tsx scripts/figma/plugin/build-theme-data.ts
 */
...
// Paths
const KUMO_ROOT = join(__dirname, "../../..");
const THEME_KUMO_CSS = join(KUMO_ROOT, "src/styles/theme-kumo.css");
const BUTTON_TSX = join(KUMO_ROOT, "src/components/button/button.tsx");

/**
 * Find Tailwind's theme.css (handles pnpm structure)
 */
function getTailwindThemeCssPath(): string {
  const directPath = join(KUMO_ROOT, "node_modules/tailwindcss/theme.css");
  ...
  try {
    ...
    // Try pnpm structure
    const pnpmBasePath = join(KUMO_ROOT, "node_modules/.pnpm");
    ...
    throw new Error("Could not find tailwindcss/theme.css");
  }
}
...
// Read source files
const themeKumoCss = readFileSync(THEME_KUMO_CSS, "utf-8");
