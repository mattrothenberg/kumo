...
describe("Dropdown Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   * SNAPSHOT TESTS - Regression guards for intermediate data
   *
   * These tests capture the intermediate data (variant configs, dimensions,
   ...
   * 3. Validating the generator implementation
   *
   * If these snapshots change unexpectedly, it means:
   * - Generator implementation changed (review carefully)
   * - Layout constants changed (review carefully)
   */
  ...
  it("should produce consistent panel dimensions", () => {
    const dims = getDropdownPanelDimensions();
    expect(dims).toMatchSnapshot();
  });

  it("should produce consistent menu item layout (without shortcut)", () => {
    const layout = getMenuItemLayout(false);
    expect(layout).toMatchSnapshot();
  });

  it("should produce consistent menu item layout (with shortcut)", () => {
    const layout = getMenuItemLayout(true);
    expect(layout).toMatchSnapshot();
  });
   ...
   * dropdown.ts computes before making any Figma API calls. It's the
  ...
  it("should produce consistent intermediate data for all variants (golden path)", () => {
    ...
    expect(allData.config.openValues).toBeDefined();
    expect(allData.config.variantValues).toBeDefined();
    expect(allData.config.descriptions).toBeDefined();
