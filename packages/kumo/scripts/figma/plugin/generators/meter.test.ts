...
describe("Meter Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   * SNAPSHOT TESTS - Regression guards for intermediate data
   *
   * These tests capture the intermediate data (fill levels, dimensions, colors,
   * layout calculations) BEFORE it hits Figma APIs. This enables:
   *
   * 1. Testing without Figma plugin runtime
   * 2. Detecting unintended changes in generator logic
   * 3. Validating the source of truth chain:
   *    meter.tsx → component-registry.json → meter.ts generator → Figma
   *
   * If these snapshots change unexpectedly, it means:
   * - Meter component changed in meter.tsx (intended)
   * - Generator logic changed (review carefully)
   * - Registry generation changed (review carefully)
   */

  it("should produce consistent fill level config", () => {
    const config = getMeterFillLevelConfig();
  ...
  it("should produce consistent indicator widths for all fill levels", () => {
    const config = getMeterFillLevelConfig();
    const widths = config.fillLevels.map((level) => ({
      fillPercentage: level,
      indicatorWidth: getMeterIndicatorWidth(level),
    }));
    expect(widths).toMatchSnapshot();
  });
   ...
   * meter.ts computes before making any Figma API calls. It's the
   * most comprehensive regression guard.
   */
  it("should produce consistent intermediate data for all fill levels (golden path)", () => {
    const allData = getAllMeterVariantData();

    // Verify structure exists
    expect(allData.registry).toBeDefined();
    expect(allData.fillLevels).toBeDefined();
    expect(allData.dimensions).toBeDefined();
    expect(allData.colorBindings).toBeDefined();
    expect(allData.layout).toBeDefined();

    // Each fill level should have complete data
    for (const level of allData.fillLevels) {
      expect(level.fillPercentage).toBeDefined();
      expect(level.indicatorWidth).toBeDefined();
      expect(level.label).toBeDefined();
      expect(level.valueText).toBeDefined();
    }
