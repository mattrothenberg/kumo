...
describe("LinkButton Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   ...
   *    button.tsx → component-registry.json → link-button.ts parser → Figma
   *
  ...
  // Snapshot each size
  for (const size of sizeProp.values) {
    it(`should produce consistent parsed styles for ${size} size`, () => {
      const sizeData = getLinkButtonParsedSizeStyles(size);
   ...
   * link-button.ts computes before making any Figma API calls. It's the
