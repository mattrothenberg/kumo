...
describe("Switch Generator - Track Color Binding", () => {
  ...
  it("should use surface-3 for unchecked state", () => {
    const binding = getSwitchTrackColorBinding("default", false, false);
    expect(binding.bgVariableName).toBe("color-surface-3");
  });

  it("should use primary for checked default variant", () => {
    const binding = getSwitchTrackColorBinding("default", true, false);
    expect(binding.bgVariableName).toBe("color-primary");
  });

  it("should use error for checked error variant", () => {
    const binding = getSwitchTrackColorBinding("error", true, false);
    expect(binding.bgVariableName).toBe("color-error");
  });

  it("should use surface-3 for disabled checked state", () => {
...
describe("Switch Generator - Complete Variant Data", () => {
  ...
  it("should include thumb color as white", () => {
    const allData = getAllSwitchVariantData();
    expect(allData.thumbColor).toBeDefined();
    expect(allData.thumbColor.r).toBe(1);
    expect(allData.thumbColor.g).toBe(1);
    expect(allData.thumbColor.b).toBe(1);
  });
...
describe("Switch Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   ...
   *    switch.tsx → component-registry.json → switch.ts parser → Figma
   *
   * If these snapshots change unexpectedly, it means:
   * - Switch component styles changed in switch.tsx (intended)
  ...
  // Snapshot each size dimensions
  const sizeConfig = getSwitchSizeConfig();
  for (const size of sizeConfig.values) {
    it(`should produce consistent dimensions for ${size} size`, () => {
      const dimensions = getSwitchDimensions(size);
      expect(dimensions).toMatchSnapshot();
    });
  }

  // Snapshot key track color bindings (not all combinations to keep snapshots manageable)
  it("should produce consistent track color for unchecked default", () => {
    const binding = getSwitchTrackColorBinding("default", false, false);
    expect(binding).toMatchSnapshot();
  });

  it("should produce consistent track color for checked default", () => {
    const binding = getSwitchTrackColorBinding("default", true, false);
    expect(binding).toMatchSnapshot();
  });

  it("should produce consistent track color for checked error", () => {
    const binding = getSwitchTrackColorBinding("error", true, false);
    expect(binding).toMatchSnapshot();
  });

  it("should produce consistent track color for disabled checked", () => {
    const binding = getSwitchTrackColorBinding("default", true, true);
    expect(binding).toMatchSnapshot();
  });
   ...
   * switch.ts computes before making any Figma API calls. It's the
  ...
  it("should produce consistent intermediate data for all variants (golden path)", () => {
    ...
    // Each dimension should have complete data
    for (const dim of allData.dimensions) {
      expect(dim.size).toBeDefined();
      expect(dim.width).toBeDefined();
      expect(dim.height).toBeDefined();
      expect(dim.thumbSize).toBeDefined();
      expect(dim.padding).toBeDefined();
    }

    // Each track color should have complete data
    for (const color of allData.trackColors) {
      expect(color.variant).toBeDefined();
      expect(typeof color.checked).toBe("boolean");
      expect(typeof color.disabled).toBe("boolean");
