...
describe("MenuBar Generator - Snapshot Tests (Intermediate Data)", () => {
  /**
   * SNAPSHOT TESTS - Regression guards for intermediate data
   *
   * These tests capture the intermediate data (dimensions, options, color bindings,
   * shadow config) BEFORE it hits Figma APIs. This enables:
   *
   ...
   *    menubar.tsx → component-registry.json → menubar.ts → Figma
   *
   * If these snapshots change unexpectedly, it means:
   * - MenuBar component structure changed in menubar.tsx (intended)
  ...
  it("should produce consistent default options", () => {
    const options = getMenuBarDefaultOptions();
   ...
   * menubar.ts computes before making any Figma API calls. It's the
